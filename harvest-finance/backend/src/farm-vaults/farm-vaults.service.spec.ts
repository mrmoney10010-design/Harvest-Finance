...
describe('FarmVaultsService - amount validation', () => {
  ...
  describe('deposit()', () => {
    it('accepts a valid positive amount', async () => {
      const existing = { id: vaultId, userId, name: 'V', targetAmount: 1000, balance: 10 };
      ...
      expect(mockVaultRepo.findOne).toHaveBeenCalledWith({ where: { id: vaultId, userId } });
      expect(saved.balance).toBeDefined();
      expect(Number(saved.balance)).toBeCloseTo(60);
    });

    it('rejects zero amount', async () => {
      await expect(service.deposit(vaultId, userId, 0)).rejects.toThrow(BadRequestException);
      await expect(service.deposit(vaultId, userId, 0)).rejects.toThrow('Deposit amount must be greater than 0');
    });

    it('rejects negative amount', async () => {
      await expect(service.deposit(vaultId, userId, -1 as any)).rejects.toThrow(BadRequestException);
    });

    it('handles very small positive amounts (boundary)', async () => {
      const existing = { id: vaultId, userId, name: 'V', targetAmount: 1000, balance: 0 };
      mockVaultRepo.findOne.mockResolvedValue(existing);
      mockVaultRepo.save.mockImplementation(async (v: any) => v);
    ...
    it('handles very large amounts', async () => {
      const existing = { id: vaultId, userId, name: 'V', targetAmount: 1e18, balance: 0 };
      mockVaultRepo.findOne.mockResolvedValue(existing);
      mockVaultRepo.save.mockImplementation(async (v: any) => v);
  ...
  describe('withdraw()', () => {
    it('accepts valid positive amount and updates balance', async () => {
      const existing = { id: vaultId, userId, name: 'V', targetAmount: 1000, balance: 500 };
      mockVaultRepo.findOne.mockResolvedValue(existing);
      mockVaultRepo.save.mockImplementation(async (v: any) => v);
    ...
    it('rejects zero withdrawal', async () => {
      await expect(service.withdraw(vaultId, userId, 0)).rejects.toThrow(BadRequestException);
    });

    it('rejects negative withdrawal', async () => {
      await expect(service.withdraw(vaultId, userId, -5 as any)).rejects.toThrow(BadRequestException);
    });

    it('rejects withdrawal greater than balance', async () => {
      const existing = { id: vaultId, userId, name: 'V', targetAmount: 1000, balance: 100 };
      mockVaultRepo.findOne.mockResolvedValue(existing);
      await expect(service.withdraw(vaultId, userId, 101)).rejects.toThrow(BadRequestException);
      await expect(service.withdraw(vaultId, userId, 101)).rejects.toThrow('Insufficient balance in farm vault');
    });
    ...
    it('boundary tests around balance', async () => {
      ...
      mockVaultRepo.findOne.mockResolvedValueOnce({ id: vaultId, userId, name: 'V', targetAmount: 1e6, balance });
      ...
      mockVaultRepo.findOne.mockResolvedValueOnce({ id: vaultId, userId, name: 'V', targetAmount: 1e6, balance });
      await expect(service.withdraw(vaultId, userId, balance + 1)).rejects.toThrow(BadRequestException);
    });
  });

  it('throws NotFound when vault is missing for deposit/withdraw', async () => {
    mockVaultRepo.findOne.mockResolvedValue(null);
    await expect(service.deposit(vaultId, userId, 1)).rejects.toThrow(NotFoundException);
    await expect(service.withdraw(vaultId, userId, 1)).rejects.toThrow(NotFoundException);
  });
});
